...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServiceResponse<String> cancel(Integer userId, Long orderNo){
        Order order = orderMapper.selectOrderByUserIdOrderId(userId,orderNo);
        if(order == null){
    ...
    public ServiceResponse<OrderVo> getOrderDetail(Integer userId,Long orderNo){
        Order order = orderMapper.selectOrderByUserIdOrderId(userId,orderNo);
        if(order != null){
            List<OrderItem> orderItemList = orderItemMapper.getByUseIdOrderNo(userId,orderNo);
    ...
    private List<OrderVo> assembleOrderVoList(List<Order> orderList,Integer userId){
        ...
        for(Order order : orderList){
            ...
            if(userId == null){
                ...
                orderItemList = orderItemMapper.getByUseIdOrderNo(userId ,order.getOrderNo());
            }
    ...
    public ServiceResponse createOrder(Integer userId, Integer ShippingId){
        //获取所有购物车中被选的购物车，相当于找购物车中的被勾选产品，产品和购物车是一对一的关系
        List<Cart> cartList = cartMapper.selectCheckedCartByUserId(userId);
        //计算订单总价
        ServiceResponse serviceResponse = this.getCartOrderItem(userId,cartList);
        if(!serviceResponse.isSuccess()){
        ...
        if(order == null){
            return ServiceResponse.createByErrorMessage("创建订单失败");
        }
        //orderItemList是从购物车中拼来 getCartOrderItem方法
    ...
    private void reduceProductStock(List<OrderItem> orderItemList){
        for (OrderItem orderItem : orderItemList){
            ...
            product.setStatus(product.getStatus()-orderItem.getQuantity());
    ...
    private ServiceResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        for(Cart cartItem : cartList){
            ...
            if(Const.ProductStatusEnum.ON_SALE.getCode() != product.getStatus()){
                return ServiceResponse.createByErrorMessage("产品"+product.getName()+"不在销售状态");
            }
            ...
            orderItem.setTotalPrice(BigDecimalUtil.mul(cartItem.getQuantity().doubleValue(),
...
//                + (long) (Math.random() * 10000000L);
        ...
        String subject = new StringBuffer().append("happymmall扫码支付").append(outTradeNo).toString();
